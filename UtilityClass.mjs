...
`
	...
	Time = {
		...
		"getNextWeekStart": (function (datetime, mode = this.Time.mode) {
			var date = new Date;
			if (typeof datetime != 'undefined') date = datetime;
			var nextWeek = this.Time.addDays(date, 7);
	...
	DOMElements = {
				...
				<div class="container text-center" style="margin:0px; padding: 0px;">
					<div class="row g-0 top-row" style="margin:0; padding:0;">
						<div class="container-fluid text-center connection-container" style="overflow:hidden; max-width: 40vh;margin:0; padding: 0;">
							<i class="add-connection fa-solid fa-plus text-danger-emphasis" style="--bs-text-opacity: .8; data--connection-direction="v""></i>
							<span class="top-input"></span>
							<i class="remove-connection fa-solid fa-minus text-danger-emphasis" style="--bs-text-opacity: .8; data--connection-direction="v""></i>
						</div>
					</div>
					<div class="d-flex mid-row">
						<div class="flex d-flex align-items-center left-column" style="overflow:hidden; margin:0; padding: 0;">
							<div class="container text-start connection-container" style="width:1vx;overflow:hidden; margin:0; padding: 0;">
								<i class="add-connection fa-solid fa-plus text-warning-emphasis" style="--bs-text-opacity: .8;" data--connection-direction="h"></i><br>
								<span class="left-input connection-column" style="flex-direction: column;align-items: center;"></span>
								<i class="remove-connection fa-solid fa-minus text-warning-emphasis" style="--bs-text-opacity: .8; data--connection-direction="h""></i>
							</div>
						</div>
						<div class="flex-fill mid-column" style="margin:0vh; padding: 0;">
							<div id="${id}-header" class="raised-element container-fluid" style="padding: 10px 20px 10px 20px; cursor: move;  background-color: #2196F3; color: #fff; border-radius:8px; ">${label}</div>
							<div id="${id}-content" style="">${content}</div>
						</div>
						<div class="flex d-flex align-items-center right-column" style="overflow:hidden; margin:0; padding: 0;">
							<div class="container text-end connection-container" style="margin:0; padding:0;">
								<i class="add-connection fa-solid fa-plus text-success-emphasis" style="--bs-text-opacity: .8;" data--connection-direction="h"></i><br>
								<span class="right-output connection-column" style="flex-direction: column;align-items: center;"></span>
								<i class="remove-connection fa-solid fa-minus text-minus-emphasis" style="--bs-text-opacity: .8; data--connection-direction="h""></i><br>
							</div>
						</div>
					</div>
					<div class="row g-0 bottom-row" style="height:1vx;margin:0; padding: 0;">
						<div class="container-fluid connection-container" style="overflow:hidden; max-width: 40vh;margin:0; padding: 0;">
							<i class="add-connection fa-solid fa-plus text-success-emphasis" style="--bs-text-opacity: .8; data--connection-direction="v""></i>
							<span class="bottom-output"></span>
							<i class="remove-connection fa-solid fa-minus text-success-emphasis" style="--bs-text-opacity: .8; data--connection-direction="v""></i>
						</div>
					</div>
				</div>
			`;
		...
		"FindMousePosition": (function (event) {
			...
			let documentX = viewportX + window.scrollX;
			let documentY = viewportY + window.scrollY;

			// Find out the element that's right under the mouse cursor
			let elementUnderCursor = document.elementFromPoint(viewportX, viewportY);
			let elementX = null;
			let elementY = null;

			// Mouse coordinates relative to the element
			if (elementUnderCursor) {
				let rect = elementUnderCursor.getBoundingClientRect();
				elementX = viewportX - rect.left;
				elementY = viewportY - rect.top;
			}
			return {
		...
		"GenerateTableV2": (function (id, dataset, with_action, npage_num = 1, npage_result_number = 0) {
			...
			if (dataset !== false) {
				...
				if (ada_action) {
					var label = with_action.label || 'aksi';
		...
		"LocalStore":{
			"saveGraphToLocalStore": function (self, collection, document_id) {
				...
				self.nodes.forEach((d, i) => {
					storeNodes.push({ id: d.id, str: d.str, position: d.position });
				});
