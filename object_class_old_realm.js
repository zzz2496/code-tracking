...
class node {
   ...
   // TODO document why this constructor is empty
	...
	Dataset = {
		...
		"DataStatus": ["isActive", "isLive"], // 1 = isLive, 2 = isActive, 3 = isLocked, 4 = isDead
		"DataStatusUpdateReason": ["Some reason", "Another reason", "Some other reason"], // Array of strings, reason to update.
		"ExternalResource": [
			{
				"Image": {
					"Type": "JPG",
...
/*
	...
	REALMS> The World Tree Yggdrasil
		RR> System Logic (Source code, et all)

			^RP> System Logic
			RC> Knowledge Index []

				^RP> Knowledge Index
				RC> Business Logic 		<RS> Business Logic 	<RS> Business Logic 	<RS> Business Logic (Chains of Business Logic)	<RS> Queue
				
					^RP> Business Logic
					RC> Application Forms 	<RS> Application Forms Extended Data
		...
		"Perspectives": {
			...
			"InfoStrip": function ($fetch, tlegend, id = '') {
				if ($fetch !== false) {
					...
					for (let $fe in $fetch) {
						for (let $fld in $fetch[$fe]) {
							$str += '<tr valign="top">';
							let zstr = readable($fld);
